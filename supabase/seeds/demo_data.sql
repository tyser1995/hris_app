...
insert into public.company_settings (id, employee_code_pattern, employee_code_sequence)
...
insert into public.positions (id, title, department_id) values
  -- Human Resources
...
update public.employees set supervisor_id = '00000004-0000-0000-0000-000000000002'
where id = '00000004-0000-0000-0000-000000000003';

-- Pedro manages Michael
update public.employees set supervisor_id = '00000004-0000-0000-0000-000000000004'
where id = '00000004-0000-0000-0000-000000000005';

-- Michael manages Carlos, Maria Elena, Juan
update public.employees set supervisor_id = '00000004-0000-0000-0000-000000000005'
where id in (
...
update public.employees set supervisor_id = '00000004-0000-0000-0000-000000000009'
where id = '00000004-0000-0000-0000-000000000010';

-- Jennifer manages Angelica, Mark, Nina
update public.employees set supervisor_id = '00000004-0000-0000-0000-000000000010'
where id in (
...
update public.employees set supervisor_id = '00000004-0000-0000-0000-000000000014'
where id = '00000004-0000-0000-0000-000000000015';

-- Eduardo manages operations staff
update public.employees set supervisor_id = '00000004-0000-0000-0000-000000000015'
where id in (
...
update public.departments set head_id = '00000004-0000-0000-0000-000000000002'
where id = '00000002-0000-0000-0000-000000000001';  -- HR → Maria Santos

update public.departments set head_id = '00000004-0000-0000-0000-000000000004'
where id = '00000002-0000-0000-0000-000000000002';  -- IT → Pedro dela Rosa

update public.departments set head_id = '00000004-0000-0000-0000-000000000009'
where id = '00000002-0000-0000-0000-000000000003';  -- Finance → Carmen Villanueva

update public.departments set head_id = '00000004-0000-0000-0000-000000000014'
...
update public.employees
set schedule_id = (select id from public.schedules where name = 'Regular 8-5' limit 1);

-- Janitor uses split shift
update public.employees
set schedule_id = (select id from public.schedules where name = 'Janitor Split' limit 1)
...
declare
  ...
  mins_out   int;
begin
  select id into sched_id from public.schedules where name = 'Regular 8-5' limit 1;
  ...
  loop
    for emp in
      select id from public.employees where employment_status = 'active'
    loop
...
begin
  ...
  insert into public.attendance (employee_id, date, time_in, status, source, schedule_id)
  select
    id, current_date,
  ...
  -- Late (2 employees)
  insert into public.attendance (employee_id, date, time_in, late_minutes, status, source, schedule_id)
  values
  ...
  -- Absent (2 employees)
  insert into public.attendance (employee_id, date, status, source, schedule_id)
  values
...
insert into public.leave_balances (employee_id, year, leave_type, total_days, used_days)
select
  e.id,
...
insert into public.notifications (user_id, type, title, body, is_read, metadata, created_at)
